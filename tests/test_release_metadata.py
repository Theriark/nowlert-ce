"""Release metadata invariants for Nowlert CE v3.1.1."""
...
def test_changelog_preserves_history_and_adds_v311():
...
def test_v300_and_v310_release_documents_remain_historical():
    ...
    assert "current stable release is **v3.1.1**" in docker_hub.casefold()
    assert "theriark/nowlert-ce:3.1.1" in docker_hub
...
def test_production_defaults_are_versioned_and_compatible():
    ...
    assert "theriark/nowlert-ce:3.1.1" in compose
    assert "NOWLERT_IMAGE" in compose
    assert "NOWLERT_EXTERNAL_BACKUP_DIR" in compose
    assert "/nowlert/external-backups" in compose


def test_release_notes_cover_v311_compatibility_and_rollback():
    notes = (ROOT / "docs" / "releases" / "v3.1.1.md").read_text(
        encoding="utf-8"
    )

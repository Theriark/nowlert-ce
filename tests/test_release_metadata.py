"""Release metadata invariants for v2.5.5."""
...
def test_application_version_is_v255():
    assert VERSION == "2.5.5"


def test_readme_stable_and_next_release_are_current():
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    assert "stable-v2.5.5-blue" in readme
    assert "| **Current Stable Release** | **v2.5.5** |" in readme
...
def test_changelog_contains_dated_release():
    ...
    assert changelog.index("## Unreleased") < changelog.index("## 2.5.5")


def test_release_notes_and_docker_hub_metadata_are_current():
    notes = ROOT / "docs" / "releases" / "v2.5.5.md"
    ...
    assert "current stable release is **v2.5.5**" in docker_hub
...
def test_release_notes_cover_upgrade_rollback_and_acceptance():
    ...
    assert "teams_payload_too_large" in notes

# Nowlert v2.5.5 release notes

Released 27 July 2026.

Nowlert v2.5.5 is a production corrective release for Microsoft Teams
destination tests and source icons.

## Microsoft Teams delivery

The v2.5.4 generic destination-test card embedded the 30,813-byte Nowlert
...
Release images pin `NOWLERT_TEAMS_ICON_BASE_URL` to the immutable tagged
...
2. Deploy `fortpt/nowlert:2.5.5` without changing volumes or credentials.
3. Confirm the application reports version 2.5.5 and schema 9.
4. Send a generic Nowlert test to Microsoft Teams.
...
Teams fetches them independently of the Nowlert container.
...
Nowlert v2.5.5 retains schema 9, the `platform_database_v1` model, and the

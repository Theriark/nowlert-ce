# Nowlert v2.5.3 release notes

Released 27 July 2026.

Nowlert v2.5.3 is a schema-compatible corrective release for Synology input
...
- `NOWLERT_ICON_BASE_URL` and its build argument are removed.
- `NOWLERT_ICON_DIR` is the local-directory override.
- `NOWLERT_DISCORD_ICON_DIR` remains a compatible Discord fallback.

## Upgrade

1. Back up matched `config`, `state`, and secret directories.
2. Pull `fortpt/nowlert:2.5.3`.
3. Change only the image tag.
4. Start Nowlert and confirm schema 8.

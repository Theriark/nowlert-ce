...
`/api/health` continues to return normal health JSON. The health response is
explicitly marked `no-store` for browser and CDN caches so a tokenized image
request cannot be collapsed onto cached health JSON. The public origin comes
